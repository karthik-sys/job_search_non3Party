...
export async function POST(req:NextRequest){const {text=""}=await req.json() as {text?:string};if(text.length<80)return NextResponse.json({error:"Paste at least a short résumé summary."},{status:400});if(text.length>30000)return NextResponse.json({error:"Résumé text is too long."},{status:400});const lower=text.toLowerCase();const suggestions=tracks.map(([label,terms],i)=>{const signals=terms.filter(t=>lower.includes(t));return{id:String(i),label,score:signals.length,signals:signals.slice(0,5)}}).filter(x=>x.score).sort((a,b)=>b.score-a.score).slice(0,5);return NextResponse.json({suggestions:suggestions.length?suggestions:[{id:"general",label:"General Software Engineering",score:1,signals:["software experience"]}],privacy:"Analyzed transiently. Résumé text was not stored."})}
