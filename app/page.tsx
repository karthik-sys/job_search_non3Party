...
export default function Home() {
  ...
  const filtered = useMemo(() => {
    ...
    const result = jobs.filter((j) => {
      ...
      return (!q || haystack.includes(q)) && (source === "All sources" || j.source === source) && (sector === "All sectors" || j.sector === sector) && (!remoteOnly || j.remote) && (mode === "All roles" || j.category === mode) && (!interestOnly || matchesInterest(j, preferences));
    });
    ...
    return result;
  }, [query, source, sector, remoteOnly, mode, sort, interestOnly, preferences]);
  ...
  const companyGroups = useMemo(() => Array.from(new Set(filtered.map((j) => j.company))).map((company) => {
    ...
    return { company, jobs: companyJobs, sector: companyJobs[0].sector, source: companyJobs[0].source, byRole, interestMatches: companyJobs.filter((job) => matchesInterest(job, preferences)).length, remote: companyJobs.filter((j) => j.remote).length };
  ...
  return (
    <main>
      ...
      <section className="hero" id="top">
        ...
        <p>All kinds of US roles pulled from official company careers systems, with optional personalization when you want the index to tilt toward your interests.</p>
        ...
        <div className="stats">
          <div><strong>{jobs.length}</strong><span>US open roles</span></div>
          ...
          <div><strong>{appliedJobs.length || "-"}</strong><span>applied roles</span></div>
        </div>
      </section>
      ...
      <section className="workspace">
        <aside>
          ...
          <p className="label space">COMPANY SIZE</p>
          {sizes.map((x) => <button key={x} className={size === x ? "filter active" : "filter"} onClick={() => setSize(x)}>{x}<span>{x === "All sizes" ? registry.length : registry.filter((c) => c.size === x).length}</span></button>)}
          ...
          <div className="fantastic"><div><span className="pulse"></span><b>Company universe mode</b></div><p>5,000 tracked companies across small, medium, large, and enterprise strata. This view loads a fast preview while the full registry powers crawler expansion.</p></div>
        </aside>
        ...
        <div className="results">
          ...
          {view === "companies" && <div className="companyGrid">{companyGroups.map((group) => <details className="companyCard" key={group.company}>
            <summary><div className="companyMonogram">{group.company.slice(0, 2)}</div><div className="companyIdentity"><p className="label">{group.sector}</p><h3>{group.company}</h3><span>Official {group.source.replace("Direct ", "")} careers feed</span></div><div className="companyCount"><strong>{group.jobs.length}</strong><span>open roles</span></div><span className="expand">+</span></summary>
          ...
          {view === "applied" && <div className="appliedPanel">
            <div className="gmailCta"><div><p className="label">OPTIONAL EMAIL SYNC</p><h3>Track application updates from Gmail</h3><span>For this demo, Gmail access is user-triggered. The production app should use OAuth and let users revoke/delete imports.</span></div><button className="ghost" onClick={() => setShowGmail(true)}>Connect / import</button></div>
      ...
      {showGmail && <div className="drawerBackdrop" onClick={() => setShowGmail(false)}><section className="onboard" onClick={(e) => e.stopPropagation()}><button className="close" onClick={() => setShowGmail(false)}>×</button><p className="eyebrow">OPTIONAL GMAIL SYNC</p><h2>Email-aware tracking.</h2><p>This demo surface is ready for Gmail imports: confirmations, assessments, interview invites, offers, and rejections can be attached to applied roles. Gmail access should always be opt-in, read-only by default, revocable, and deletable from the tracker.</p><div className="gmailMock"><div><b>Detected update types</b><span>Application received · Assessment · Interview · Offer · Rejection</span></div><div><b>Privacy controls</b><span>Manual import · Delete imported email metadata · Disconnect anytime</span></div></div><button className="apply">Import recent application emails</button><small>The live Sites app cannot directly use the Codex Gmail connector yet. In production this button should start a user-owned OAuth flow; in this Codex session I can test Gmail reads only when you explicitly ask.</small></section></div>}
    </main>
  );
}
...
function JobCard({ job, applied, onOpen, onApply }: { job: Job; applied: boolean; onOpen: () => void; onApply: () => void }) {
  return <article className="job" onClick={onOpen} tabIndex={0} onKeyDown={(e) => e.key === "Enter" && onOpen()}>
    ...
    <div className="jobMain"><div className="jobTop"><h3>{job.title}</h3><span>{fmtDate(job.date)}</span></div><p className="company">{job.company} <i>·</i> {job.location}</p><p className="summary">{job.summary}</p><div className="chips"><span className="category">{job.category}</span><span className="hq">{job.sector}</span><span className="source">Official careers</span>{job.remote && <span>Remote</span>}{applied && <span className="appliedChip">Applied</span>}</div></div>
